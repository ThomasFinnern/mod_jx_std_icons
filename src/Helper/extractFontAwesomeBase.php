...
abstract class extractFontAwesomeBase
{
     ...
     * @since version
     */
    public function extractScssIconFileData($brandsPathFileName = self::CSS_JOOMLA_SYSTEM_PATH_FILE_NAME)
    {
        $brandNames = [];
        ...
        try
        {
            ...
            if (!is_file($brandsPathFileName))
            {
                ...
                $OutTxt = 'Warning: extractScssIconFileData: File does not exist "' . $brandsPathFileName . '"<br>';
            ...
            else
            {
                ...
                $scssIconValues = $this->lines_collectScssIconValues($lines);
                // sort($brandNames);
                //--- standard icon and brand names ------------------------------------------

                [$stdandardNames, $brandNames] = $this->lines_collectScssIconNames($lines);
                // sort($brandNames);
            }
        }
        catch (\RuntimeException $e)
        {
            $OutTxt = '';
            $OutTxt .= 'Error executing extractScssIconFileData: "' . '<br>';
            $OutTxt .= 'Error: "' . $e->getMessage() . '"' . '<br>';
    ...
    public function lines_collectScssIconNames(array $lines)
    {
        ...
        $OutTxt = Text::_('lines_collectScssIconNames must be overwritten');

        $app = Factory::getApplication();
        $app->enqueueMessage($OutTxt, 'warning');

        return [$stdandardNames, $brandNames];
    }
    ...
    public function lines_collectScssIconValues(array $lines)
    {
        ...
        $OutTxt = Text::_('lines_collectScssIconValues must be overwritten');
